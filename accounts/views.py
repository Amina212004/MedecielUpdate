    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            send_mail(
                ...
                from_email='medeciels@gmail.com',  # Updated to match settings
...
class AdminAddUserView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            serializer.validated_data['created_by_admin'] = True  # Set admin-created flag
            user = serializer.save()
            return Response({
                ...
                'user': {
                    ...
                    'img': user.img or f"https://randomuser.me/api/portraits/{'men' if user.id % 2 else 'women'}/{user.id % 100}.jpg"
                }
            }, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
class RecentUsersView(APIView):
    ...
    def get(self, request):
        users = CustomUser.objects.filter(created_by_admin=True).order_by('-date_joined')[:4]
        serializer = UserSerializer(users, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)