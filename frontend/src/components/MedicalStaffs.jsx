...
const MedicalStaffs = () => {
  ...
  const [filter, setFilter] = useState('All'); // Filter state: All, Medecin, Assistant
  const [showDropdown, setShowDropdown] = useState(false);
  const pageSize = 10;

  const fetchData = async (page = 1, role = filter) => {
    try {
      ...
      const usersResponse = await axios.get(`http://localhost:8000/api/medical-staff/?page=${page}${roleQuery}`, {
        headers: {
      ...
      // Fetch current user (only on initial load)
      let userResponse = null;
      if (!currentUser) {
        userResponse = await axios.get('http://localhost:8000/api/current-user/', {
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Token ${token}`,
          },
      ...
      setNextPage(usersResponse.data.next);
      setPrevPage(usersResponse.data.previous);
  ...
  const handleFilterChange = (role) => {
    ...
    fetchData(1, role); // Reset to page 1 with new filter
  };
        ...
        >
          <div className="flex-1 flex flex-col p-8 overflow-auto bg-white rounded-[10px] gap-20">
            ...
            >
              <div>
                ...
                <p className="text-[20px]">
                  You can now manage medical staff, oversee system activity, and ensure everything runs smoothly. Have a great day!
                </p>
              </div>
            ...
            <div>
                    ...
                    <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-[#002C4E] hover:bg-[#95BFC3] ${
                          filter === 'All' ? 'bg-[#1B9C92] text-white' : ''
                        }`}
                      >
                        All
                      </button>
                      <button
                        onClick={() => handleFilterChange('Medecin')}
                        className={`w-full text-left px-4 py-2 text-[#002C4E] hover:bg-[#95BFC3] ${
                          filter === 'Medecin' ? 'bg-[#1B9C92] text-white' : ''
                        }`}
                      >
                        Medecin
                      </button>
                      <button
                        ...
                        className={`w-full text-left px-4 py-2 text-[#002C4E] hover:bg-[#95BFC3] ${
                          filter === 'Assistant' ? 'bg-[#1B9C92] text-white' : ''
                        }`}
                      >
                        Assistant
                      </button>
              ...
              {loading && <p className="text-[20px] text-gray-500">Loading staff...</p>}
              {error && <p className="text-[20px] text-red-500">{error}</p>}
              {!loading && !error && users.length === 0 && (
                <p className="text-[20px] text-gray-500">No medical staff found.</p>
              )}
              {!loading && !error && users.length > 0 && (
                <div>
                  ...
                  <div className="flex justify-center items-center gap-2 mt-4">
                    <button
                      onClick={() => prevPage && fetchData(currentPage - 1, filter)}
                      disabled={!prevPage}
                      className={`px-4 py-2 rounded ${!prevPage ? 'bg-gray-300' : 'bg-[#1B9C92] text-white hover:bg-[#165E72]'}`}
                    >
                      Previous
                    </button>
                    {pageNumbers.map((page) => (
                      <button
                        ...
                        className={`px-4 py-2 rounded ${
                          currentPage === page
                            ? 'bg-[#1B9C92] text-white'
                            : 'bg-gray-200 text-[#002C4E] hover:bg-[#95BFC3]'
                        }`}
                      >
                        {page}
                      </button>
                    ...
                    <button
                      onClick={() => nextPage && fetchData(currentPage + 1, filter)}
                      disabled={!nextPage}
                      className={`px-4 py-2 rounded ${!nextPage ? 'bg-gray-300' : 'bg-[#1B9C92] text-white hover:bg-[#165E72]'}`}
                    >
                      Next
                    </button>
                  </div>
